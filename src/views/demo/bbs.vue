  ...
  <div>
    ...
    <van-row class="bbs" v-for="(item,i) in list" :key="item.dynamic_id">
      ...
      <van-col span="18">
        ...
        <van-row>
          <van-col span="6">
            <van-icon size="1.2em" :name="item.name" @click="addGood(item.dynamic_id,item.good,i)"/>
            {{item.good}}
          </van-col>
          <van-col span="8">
            <p>{{item.city}}</p>
          ...
          <van-col span="9" style=" text-align: right;">
            <p>{{item.createdAt|currentTime()}}</p>
          </van-col>
        </van-row>
        <van-field v-model="item.content" type="textarea" rows="2" autosize readonly/>
      </van-col>
    </van-row>
    <div class="gap">
      <p>没有更多了，亲！</p>
    ...
    addGood: function(dynamic_id, good, i) {
      ...
      putDynamic(dynamic_id, {
        ...
        .then(res => {
          // console.log(res);
          if (res.updatedAt) {
            this.list[i].good = this.list[i].good + temp; //使点赞数+或-1
